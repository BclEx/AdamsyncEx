...
namespace System.Text.Lalr
{
    ...
    public class Parser
    {
        ...
        private static void ParseSingleToken(string z, Context ctx, ParserContext pctx)
        {
            ...
            switch (pctx.FiniteState)
            {
                ...
                case FiniteState.WAITING_FOR_DECL_OR_RULE:
                    ...
                    {
                        pctx.LHSymbol = Symbol.New(ctx, v);
                    ...
                    else if (v[0] == '{')
                    {
                        if (pctx.PrevRule == null)
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "There is no prior rule opon which to attach the code fragment which begins on this line.");
                        else if (pctx.PrevRule.Code != null)
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Code fragment beginning on this line is not the first to follow the previous rule.");
                        else
                        {
                        ...
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Token \"{0}\" should be either \"%\" or a nonterminal name.", v);
                    break;
                case FiniteState.PRECEDENCE_MARK_1:
                    if (!char.IsUpper(v[0]))
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "The precedence symbol must be a terminal.");
                    else if (pctx.PrevRule == null)
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "There is no prior rule to assign precedence \"[{0}]\".", v);
                    else if (pctx.PrevRule.PrecedenceSymbol != null)
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Precedence mark on this line is not the first to follow the previous rule.");
                    else
                        pctx.PrevRule.PrecedenceSymbol = Symbol.New(ctx, v);
                    pctx.FiniteState = FiniteState.PRECEDENCE_MARK_2;
                    break;
                case FiniteState.PRECEDENCE_MARK_2:
                    if (v[0] != ']')
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Missing \"]\" on precedence mark.");
                ...
                case FiniteState.WAITING_FOR_ARROW:
                    ...
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Expected to see a \":\" following the LHS symbol \"{0}\".", pctx.LHSymbol.Name);
                ...
                case FiniteState.LHS_ALIAS_1:
                    ...
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "\"{0}\" is not a valid alias for the LHS \"{1}\"\n", v, pctx.LHSymbol.Name);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_RULE_ERROR;
                    }
                    break;
                case FiniteState.LHS_ALIAS_2:
                    ...
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Missing \")\" following LHS alias name \"{0}\".", pctx.LHSymbolAlias);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_RULE_ERROR;
                    }
                    break;
                case FiniteState.LHS_ALIAS_3:
                    ...
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Missing \".\" following: \"{0}({1})\".", pctx.LHSymbol.Name, pctx.LHSymbolAlias);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_RULE_ERROR;
                    }
                    break;
                case FiniteState.IN_RHS:
                    ...
                    else if (char.IsLetter(v[0]))
                    {
                        ...
                        {
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Too many symbols on RHS of rule beginning at \"{0}\".", v);
                            pctx.FiniteState = FiniteState.RESYNC_AFTER_RULE_ERROR;
                        }
                        else
                        {
                            pctx.QueuedRHSymbol[pctx.QueuedRHSymbols] = Symbol.New(ctx, v);
                    ...
                    else if ((v[0] == '|' || v[0] == '/') && pctx.QueuedRHSymbols > 0)
                    {
                        ...
                        multiSymbol.Children[multiSymbol.Children.Length - 1] = Symbol.New(ctx, v.Substring(1));
                        if (char.IsLower(v[1]) || char.IsLower(multiSymbol.Children[0].Name[0]))
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Cannot form a compound containing a non-terminal");
                    }
                    else if (v[0] == '(' && pctx.QueuedRHSymbols > 0)
                        pctx.FiniteState = FiniteState.RHS_ALIAS_1;
                    else
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Illegal character on RHS of rule: \"{0}\".", v);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_RULE_ERROR;
                    }
                    break;
                case FiniteState.RHS_ALIAS_1:
                    ...
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "\"{0}\" is not a valid alias for the RHS symbol \"{1}\"\n", v, pctx.QueuedRHSymbol[pctx.QueuedRHSymbols - 1].Name);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_RULE_ERROR;
                    }
                    break;
                ...
                case FiniteState.WAITING_FOR_DECL_KEYWORD:
                    if (char.IsLetter(v[0]))
                    {
                        ...
                        {
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Unknown declaration keyword: \"%{0}\".", v);
                            pctx.FiniteState = FiniteState.RESYNC_AFTER_DECL_ERROR;
                        }
                    }
                    else
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Illegal declaration keyword: \"{0}\".", v);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_DECL_ERROR;
                    }
                    break;
                case FiniteState.WAITING_FOR_DESTRUCTOR_SYMBOL:
                    if (!char.IsLetter(v[0]))
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Symbol name missing after %destructor keyword");
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_DECL_ERROR;
                    }
                    else
                    {
                        var symbol = Symbol.New(ctx, v);
                ...
                case FiniteState.WAITING_FOR_DATATYPE_SYMBOL:
                    if (!char.IsLetter(v[0]))
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Symbol name missing after %type keyword");
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_DECL_ERROR;
                    }
                    else
                    {
                        Symbol symbol;
                        if (!ctx.AllSymbols.TryGetValue(v, out symbol))
                            symbol = null;
                        if (symbol != null && symbol.DataType != null)
                        {
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Symbol %type \"{0}\" already defined", v);
                            pctx.FiniteState = FiniteState.RESYNC_AFTER_DECL_ERROR;
                        }
                        else
                        {
                            if (symbol == null)
                                symbol = Symbol.New(ctx, v);
                ...
                case FiniteState.WAITING_FOR_PRECEDENCE_SYMBOL:
                    ...
                    else if (char.IsLetter(v[0]))
                    {
                        ...
                        if (symbol.Precedence >= 0)
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Symbol \"{0}\" has already be given a precedence.", v);
                        else
                        {
                        ...
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Can't assign a precedence to \"{0}\".", v);
                    break;
                case FiniteState.WAITING_FOR_DECL_ARG:
                    if (v[0] == '{' || v[0] == '"' || char.IsLetterOrDigit(v[0]))
                    {
                        ...
                        if (addLineMacro)
                        {
                            ...
                            zBuf += string.Format("#line {0} \"{1}\"\n", pctx.TokenLineno, ctx.InputFilePath.Replace("\\", "\\\\"));
                        }
                    ...
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Illegal argument to %{0}: {1}", pctx.DeclKeyword, v);
                        pctx.FiniteState = FiniteState.RESYNC_AFTER_DECL_ERROR;
                    }
                    break;
                case FiniteState.WAITING_FOR_FALLBACK_ID:
                    if (v[0] == '.')
                        ...
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "%fallback argument \"{0}\" should be a token", v);
                    else
                    {
                        var symbol = Symbol.New(ctx, v);
                        if (pctx.FallbackSymbol == null)
                            ...
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "More than one fallback assigned to token {0}", v);
                        else
                        {
                ...
                case FiniteState.WAITING_FOR_WILDCARD_ID:
                    if (v[0] == '.')
                        ...
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "%wildcard argument \"{0}\" should be a token", v);
                    else
                    {
                        var symbol = Symbol.New(ctx, v);
                        if (ctx.Wildcard == null)
                            ...
                            ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "Extra wildcard to token: {0}", v);
                    }
                    break;
        ...
        /// <summary>
        /// Parses the specified CTX.
        /// </summary>
        /// <param name="ctx">The CTX.</param>
        /// <param name="path">The path.</param>
        /// <param name="z">The z.</param>
        /// <param name="defines">The defines.</param>
        public static void Parse(Context ctx, string path, string z, string[] defines)
        {
            var startline = 0;
            ...
            for (var cp = 0; cp < z.Length && (c = z[cp]) != 0; )
            {
                ...
                if (c == '\"')
                {
                    ...
                    if (cp < z.Length)
                    {
                        ctx.RaiseError(ref pctx.Errors, startline, "String starting on this line is not terminated before the end of the file.");
                        nextcp = cp;
                    }
                    else
                        nextcp = cp + 1;
                ...
                else if (c == '{')
                {
                    ...
                    if (cp == z.Length)
                    {
                        ctx.RaiseError(ref pctx.Errors, pctx.TokenLineno, "C code starting on this line is not terminated before the end of the file.");
                        nextcp = cp;
                    }
                    else
                        nextcp = cp + 1;
            ...
            ctx.Symbols = Symbol.ToSymbolArray(ctx, out ctx.Terminals);
            ctx.Ensure();
        }
    }
}
